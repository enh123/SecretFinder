...
def get_pattern_description():
    pattern_description = [
       ...
       # "ip 地址"),
        ...
        (re.compile(
        r'''\b(?=[0-9.]{9,}\b)'''                                   # 关键修复：强制接下来的9个字符只能是数字或点号
        r'''(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])\.'''      # 第1段
        r'''(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])\.'''      # 第2段
        r'''(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])\.'''      # 第3段
        r'''(?:25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9]?[0-9])\b'''      # 第4段
        ),"ip 地址"),
        ...
        (re.compile(r'''(?i)["']?DingtalkOpenid["']?\s*[:=]\s*(?:(["'])(.*?)\1|([^"'\s]+))'''), "DingtalkOpenid"),
        (re.compile(
    r'\btype\s*[:=]\s*["\']file["\']|\.files\b|\bnew\s+FormData\b|\bnew\s+FileReader\b|\baccept\s*=\s*["\']|\bdataTransfer\.files\b'),"upload form"),

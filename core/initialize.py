...
def parse_args():
    ...
    parser.add_argument("-p", "--proxy", "-proxy", dest="proxy", help="设置代理,例如:--proxy=http://127.0.0.1:8080",
                        required=False)
    parser.add_argument("-po", "--path-only", help="只提取路径",action="store_true",required=False)
    parser.add_argument("-s", "--stdin", dest="stdin",
                        help="从标准输入读取内容并查找敏感信息",
                        required=False, action="store_true")
    parser.add_argument("-key", "--key", dest="key",
                        help="提取query中的key",
                        required=False, action="store_true")
    parser.add_argument("-value", "--value", dest="value",
                        help="提取query中的value",
                        required=False, action="store_true")
    ...
    if not sys.stdin.isatty(): #判断标准输入是来自终端还是管道或重定向输入
        try:
            stdin_text = sys.stdin.read().strip()
            if stdin_text:
                config.set_stdin_text(stdin_text)
        except:
            pass
